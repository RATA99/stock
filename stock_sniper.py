...
def build_prompt(symbol, interval_label, df, fibo, current_p, ema50, ema200):
    ...
    return f"""🛡️ High-Precision Sniper Stock Analysis (Thai Market)

Role: คุณคือผู้เชี่ยวชาญการวิเคราะห์หุ้นไทย (SET) สาย Technical ผสม Quantitative เน้นความแม่นยำสูง (High Win Rate) ใช้กลยุทธ์ Multi-Timeframe (MTF) หาจุด Confluence ของราคา

═══════════════════════════════
📊 Real-Time Data: {symbol} ({interval_label})
═══════════════════════════════
- ราคาปัจจุบัน : {current_p:,.2f} บาท ({chg_pct:+.2f}% จากแท่งก่อน)
...
- Volume ล่าสุด: {last_vol:,.0f} หุ้น ({vol_ratio:.1f}x เทียบ avg 20 แท่ง)
- แนวรับ Statistical: {', '.join([f'{x:,.0f}' for x in support_levels])}
- 10 แท่งล่าสุด: {json.dumps(last10, default=str, ensure_ascii=False)}

═══════════════════════════════
วิเคราะห์ตาม Framework นี้อย่างเคร่งครัด ใช้ภาษาไทย กระชับ เน้นตัวเลข:

**1. 🔭 Macro Analysis — โครงสร้างและแนวโน้มหลัก**
- Trend Validation: EMA 50 vs EMA 200 → Golden Cross / Death Cross + ความหมาย
- Fibonacci Confluence: ราคาอยู่ที่ระดับ Fibo ใด และมีนัยอย่างไร
- Major Levels: แนวรับ-แนวต้านหลักที่มีนัยสำคัญทางสถิติ

**2. ⚡ Micro Execution — จังหวะและโมเมนตัม**
- Volume Price Analysis (VPA): Volume Spike เทียบ avg 20 แท่ง → แปลว่าอะไร
- Price Pattern: Reversal / Continuation Pattern ที่เห็นในกราฟ (Double Bottom, Bull Flag, ฯลฯ)
- Candlestick Signal: แท่งเทียนจุดกลับตัวล่าสุด (Bullish Engulfing, Hammer, Pin Bar)

**3. 🎯 Strategic Summary — The Action Plan**

Confluence Verdict: สัญญาณ Macro + Micro ไปทิศทางเดียวกันหรือไม่ (ถ้าไม่ตรงให้บอก "Wait & See")
...
Risk/Reward Ratio: คำนวณให้ได้ไม่ต่ำกว่า 1:2 (ถ้าไม่ถึงให้บอกว่า Skip)"""
